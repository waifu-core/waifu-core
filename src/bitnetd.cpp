...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/bitnet.conf are parsed in qt/bitnet.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bitnetd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitnetd -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for bitnetd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect bitnetd signal handlers
    noui_connect();
