...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Waifu address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new WaifuAddressEntryValidator(parent));
    widget->setCheckValidator(new WaifuAddressCheckValidator(parent));
}
...
bool parseWaifuURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!WaifuUnits::parse(WaifuUnit::BIT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseWaifuURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseWaifuURI(uriInstance, out);
}
...
//    QString ret = QString("waifu:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WaifuUnits::format(WaifuUnit::BIT, info.amount, false, WaifuUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Waifu.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Waifu (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Waifu (%s).lnk", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Waifu\n";
        else
            optionFile << strprintf("Name=Waifu (%s)\n", chain);
