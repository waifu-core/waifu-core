// Copyright (c) 2011-2022 The Waifu Core developers
...
const std::string WaifuGUI::DEFAULT_UIPLATFORM =
...
WaifuGUI::WaifuGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &WaifuGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &WaifuGUI::showModalOverlay);
...
void WaifuGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Waifu address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and waifu: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &WaifuGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &WaifuGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &WaifuGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Waifu addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Waifu addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Waifu Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Waifu Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a waifu: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Waifu command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &WaifuGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &WaifuGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &WaifuGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &WaifuGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &WaifuGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &WaifuGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &WaifuGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &WaifuGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &WaifuGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &WaifuGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &WaifuGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &WaifuGUI::showDebugWindow);
}

void WaifuGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void WaifuGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &WaifuGUI::setCurrentWalletBySelectorIndex);
...
void WaifuGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &WaifuGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &WaifuGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &WaifuGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &WaifuGUI::showProgress);
...
#ifdef ENABLE_WALLET
void WaifuGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &WaifuGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &WaifuGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* WaifuGUI::getWalletController()
{
    return m_wallet_controller;
}

void WaifuGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &WaifuGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &WaifuGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &WaifuGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &WaifuGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &WaifuGUI::incomingTransaction);
    connect(this, &WaifuGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void WaifuGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void WaifuGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void WaifuGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void WaifuGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void WaifuGUI::setWalletActionsEnabled(bool enabled)
{
...
void WaifuGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &WaifuGUI::toggleHidden);
...
void WaifuGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void WaifuGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void WaifuGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void WaifuGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void WaifuGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void WaifuGUI::openClicked()
{
...
void WaifuGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void WaifuGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void WaifuGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void WaifuGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void WaifuGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void WaifuGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void WaifuGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void WaifuGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Waifu network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void WaifuGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void WaifuGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void WaifuGUI::updateHeadersSyncProgressLabel()
{
...
void WaifuGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void WaifuGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &WaifuGUI::quitRequested);
...
void WaifuGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void WaifuGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void WaifuGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &WaifuGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &WaifuGUI::show);
                e->ignore();
            }
        }
    }
...
void WaifuGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void WaifuGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void WaifuGUI::incomingTransaction(const QString& date, WaifuUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(WaifuUnits::formatWithUnit(unit, amount, true));
...
void WaifuGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void WaifuGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool WaifuGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool WaifuGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void WaifuGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void WaifuGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void WaifuGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void WaifuGUI::updateWindowTitle()
{
...
void WaifuGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void WaifuGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void WaifuGUI::detectShutdown()
{
...
void WaifuGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void WaifuGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(WaifuGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = WaifuGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void WaifuGUI::subscribeToCoreSignals()
{
...
void WaifuGUI::unsubscribeFromCoreSignals()
{
...
bool WaifuGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<WaifuUnit> units = WaifuUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const WaifuUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, WaifuUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const WaifuUnit u : WaifuUnits::availableUnits()) {
        menu->addAction(WaifuUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(WaifuUnit newUnits)
{
    setText(WaifuUnits::longName(newUnits));
}
