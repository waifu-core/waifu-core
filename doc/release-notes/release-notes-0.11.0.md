Waifu Core version 0.11.0 is now available from:

  <https://bitnet.org/bin/bitnet-core-0.11.0/>
...
For example, add the following to `bitnet.conf`:
...
related to the blockchain in the bitnet system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Waifu Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Waifu Core will 
...
For an example script see [bitnet-submittx](https://github.com/laanwj/bitnet-submittx).
...
- #5334 `cf87536` libbitnetconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitnet-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only WaifuAmountField
- #5753 `0f44672` Add bitnet logo to about screen
...
- #5809 `46bfbe7` Add bitnet-cli man page
...
- #5849 `41259ca` contrib/init/bitnetd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Waifu-Qt.app to Waifu-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitnet-tx)
