Waifu Core version 0.18.0 is now available from:
  ...
  <https://waifucore.org/bin/waifu-core-0.18.0/>
...
`/Applications/Waifu-Qt` (on Mac) or `waifud`/`waifu-qt` (on
Linux).
...
`contrib/init/waifud.service`, it has been changed to use
`/var/lib/waifud` as the data directory instead of
`~waifu/.waifu`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/waifud` will
exist has enough space (check using `df -h /var/lib/waifud`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/waifu/waifu/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://waifucore.org/en/contact/) the Waifu Core
...
The systemd init file (`contrib/init/waifud.service`) has been changed
to use `/var/lib/waifud` as the data directory instead of
`~waifu/.waifu`. This change makes Waifu Core more consistent with
...
`datadir` option in `/etc/waifu/waifu.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/waifu/waifu.conf`.


Documentation
-------------

- A new short [document](https://github.com/waifu/waifu/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/waifu/waifu/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/waifu/waifu/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/waifu/waifu/blob/master/doc/waifu-conf.md)
  about the `waifu.conf` file describes how to use it to configure
  Waifu Core.

- A new document introduces Waifu Core's BIP174 [Partially-Signed
  Waifu Transactions
  (PSBT)](https://github.com/waifu/waifu/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/waifu/waifu/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/waifu/waifu/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/waifu/waifu/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/waifu/waifu/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/waifu/waifu/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart waifud with the `-deprecatedrpc=generate`
...
- A new `waifu-wallet` tool is now distributed alongside Waifu
...
- A sub-project of Waifu Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/waifu-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `waifu_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libwaifu_server.a` (jnewbery)
...
- #13926 [Tools] waifu-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like waifud (MarcoFalke)
...
- #14521 qt, docs: Fix `waifu-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libwaifu_server` (laanwj)
...
- #15154 Configure: waifu-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/waifu-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libwaifu_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in waifu-util-test.py (isghe)
...
- #12246 Bugfix: Only run waifu-tx tests when waifu-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_waifu` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_waifu` (MarcoFalke)
...
- #14186 waifu-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_waifu.conf` message on startup if conf file exists (leishman)
...
- #12255 Update waifu.service to conform to init.md (dongcarl)
...
- #14497 Add doc/waifu-conf.md (hebasto)
...
- #15354 Add missing `waifu-wallet` tool manpages (MarcoFalke)
...
- #15765 correct waifuconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support waifu folder on OSX (JimmyMow)
