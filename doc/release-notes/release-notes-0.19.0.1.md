Waifu Core version 0.19.0.1 is now available from:
  ...
  <https://waifucore.org/bin/waifu-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/waifu/waifu/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `waifud -help` output for these two options for
...
- The `testnet` field in `waifu-cli -getinfo` has been renamed to
...
- #15639 waifu-wallet tool: Drop libwaifu_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in waifud/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "waifu-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of waifud build (NicolasDorier)
...
- #16441 remove qt libjpeg check from waifu_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_waifu to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_waifu output (practicalswift)
...
- #15896 feature_filelock, interface_waifu_cli: Use PACKAGE_NAME in messages rather than hardcoding Waifu Core (luke-jr)
...
- #16277 Suppress output in test_waifu for expected errors (gertjaap)
...
- #16489 log: harmonize waifud logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (waifuVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example waifu.conf (fanquake)
...
- #16080 build/doc: update waifu_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in waifu.conf (fanquake)
...
- #16621 add default waifu.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the waifu-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, waifu.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/waifud.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/waifu/waifu/).
