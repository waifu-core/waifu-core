# CJDNS support in Waifu Core

It is possible to run Waifu Core over CJDNS, an encrypted IPv6 network that
...
done outside of Waifu Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Waifu network and individual nodes.
...
before it will work with your Waifu Core node. This is described in steps
...
## Run Waifu Core with CJDNS

Once you are connected to the CJDNS network, the following Waifu Core
...
When enabled, this option tells Waifu Core that it is running in an
...
IPv6 local network. This helps Waifu Core perform better address management:
...
There are several ways to see your CJDNS address in Waifu Core:
