# I2P support in Waifu Core

It is possible to run Waifu Core as an
...
## Run Waifu Core with an I2P router (proxy)
...
Once an I2P router with SAM enabled is up and running, use the following Waifu
...
The first time Waifu Core connects to the I2P router, it automatically
...
`i2p_private_key` in the Waifu Core data directory.  The persistent I2P
...
## Fetching I2P-related information from Waifu Core

There are several ways to see your I2P address in Waifu Core if accepting
...
Waifu Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and Waifu Core

Waifu Core uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3)
...
To allow future upgrades to newer versions of SAM, Waifu Core sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of Waifu Core.
...
It's important that the nodes of a popular application like Waifu contribute
...
## Bundling I2P in a Waifu application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with Waifu.
